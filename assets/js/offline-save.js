...
(function () {
  ...
  function isSaved() {
    return getSaved().some(function (p) { return p.url === pageUrl; });
  }
  ...
  function savePost() {
    ...
    var imgs = Array.from(document.querySelectorAll('.post-content img, .post-hero__img'))
      ...
      .filter(function (src) { return src && src.startsWith(window.location.origin); });
    ...
    caches.open(SAVED_CACHE).then(function (cache) {
      return Promise.all(urls.map(function (url) {
        return fetch(url)
          ...
          .catch(function () { /* non-critical: image failed */ });
      }));
    }).then(function () {
      ...
      var saved   = getSaved().filter(function (p) { return p.url !== pageUrl; });
      saved.unshift({ url: pageUrl, title: title, date: date, savedAt: Date.now() });
      setSaved(saved.slice(0, 30));
      updateBtn(false);
    }).catch(function () { updateBtn(false); });
  }

  function removePost() {
    caches.open(SAVED_CACHE).then(function (cache) { cache.delete(window.location.href); });
    setSaved(getSaved().filter(function (p) { return p.url !== pageUrl; }));
    updateBtn(false);
  }
