...
(function () {
  ...
  var apiUrl  = 'https://webmention.io/api/mentions.jf2'
  ...
  function renderLikes(mentions) {
    ...
    var avatarsHtml = likes.map(function (m) {
      var author = m.author || {};
      var name   = escHtml(author.name || 'Anonymous');
      var url    = escHtml(author.url  || m.url || '#');
      var photo  = author.photo;
      if (photo) {
        return '<a href="' + url + '" target="_blank" rel="noopener noreferrer" title="' + name + '">' +
          '<img src="' + escHtml(photo) + '" alt="' + name + '" width="36" height="36" loading="lazy" onerror="this.parentNode.remove()">' +
          '</a>';
      }
      var initials = (author.name || 'A').charAt(0).toUpperCase();
      return '<a href="' + url + '" target="_blank" rel="noopener noreferrer" title="' + name + '" class="wm-avatar-fallback" aria-label="' + name + '">' + escHtml(initials) + '</a>';
    }).join('');
  ...
  function renderReplies(mentions) {
    ...
    var html = replies.map(function (m) {
      ...
      var avatarHtml = photo
        ? '<img src="' + escHtml(photo) + '" alt="' + name + '" width="40" height="40" loading="lazy" onerror="this.style.display=\'none\'">'
        : '<span class="wm-reply__avatar-fallback" aria-hidden="true">' + escHtml((author.name || 'A').charAt(0).toUpperCase()) + '</span>';

      return '<article class="wm-reply" role="comment">' +
        '<header class="wm-reply__header">' +
          '<a href="' + escHtml(author.url || url) + '" target="_blank" rel="noopener noreferrer" class="wm-reply__avatar" aria-label="' + name + '">' + avatarHtml + '</a>' +
          '<div class="wm-reply__meta">' +
            '<a href="' + escHtml(author.url || url) + '" target="_blank" rel="noopener noreferrer" class="wm-reply__name">' + name + '</a>' +
            (date ? '<time class="wm-reply__date" datetime="' + escHtml(m.published || '') + '">' + escHtml(date) + '</time>' : '') +
          '</div>' +
          '<a href="' + url + '" target="_blank" rel="noopener noreferrer" class="wm-reply__source" aria-label="View source">' +
  ...
  fetch(apiUrl)
    .then(function (r) { return r.ok ? r.json() : { children: [] }; })
    .then(function (data) { render(data); })
    .catch(function () { repliesEl.innerHTML = ''; });
})();
